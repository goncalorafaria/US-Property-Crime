...
def build_state_proximity_matrix():
    ...
    return Ws, isos, nisos, Ds, None, None  
...
def mcodes(region_prior,state_prior,n , y, yeari, regioni, statei):
    ...
    bvars = dict(nstates = nstates, y = y, n = n, nisor=nisor,isor=isor, Dr=Dr,Ir=Ir, Wrt=Wrt, nregions=nregions, region=region, T=T)
    ...
    model
    {
        ...
        for (t in 1:T){
            for (s in 1:nstates) {
                ...
                eps[s,t+1] <- (mu.t[s,t]*mu.effect[s,t]*n[s,t] - y[s,t])
            }
        }
    ...
    bvars = dict(nstates = nstates, y = y, n = n, nisor=nisor, isor=isor, Dr=Dr, Wr=Wr, nregions=nregions, region=region, T=T)
    ...
    model
    {
        ...
        for (t in 1:T){
            for (s in 1:nstates) {
                logit(mu.effect[s,t]) <- p + reffect[region[s]+1] + seffect[s]
                logit(mu.t[s,t]) <- t.beta[1] + t.beta[2]* eps[s,t] + t.beta[3]* eps[s,t]^2
                ...
                eps[s,t+1] <- (theta[s,t]*n[s,t] - y[s,t])
            }
        }
